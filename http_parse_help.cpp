...
int32_t ParseHostPort(BufferContext* buffer_context)
{
    ...
    do
    {
        if (buffer_context->http_request->method != METHOD_CONNECT)
        {
        ...
        {
            size_t host_begin_pos = 0;
            size_t host_end_pos = url.find_first_of(':', host_begin_pos);
            if (host_end_pos == string::npos)
            {
                LOG_ERROR("url error:" << url << " method:" << buffer_context->http_request->method);
                break;
            }
            buffer_context->remote_host = string(url, host_begin_pos, host_end_pos - host_begin_pos);
            buffer_context->remote_port = atoi(string(url, host_end_pos + 1).c_str());
        }
